...
const regexGen = /Business Generated<\/span>\s*<div[^>]*>\s*₹\{totalBusinessSent\.toLocaleString\(\)\}\s*<\/div>\s*<div[^>]*>\s*<span[^>]*>\s*\{slips\.length\} slips submitted\s*<\/span>\s*<\/div>/m;
const regexRec = /Business Received<\/span>\s*<div[^>]*>\s*₹\{totalBusinessReceived\.toLocaleString\(\)\}\s*<\/div>\s*<div[^>]*>\s*<span[^>]*>\s*\{receivedSlips\.length\} slips received\s*<\/span>\s*<\/div>/m;
...
if (found) {
  ...
  console.log('Successfully swapped labels!');
}
