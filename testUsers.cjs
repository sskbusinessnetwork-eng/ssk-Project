...
async function test() {
  const { data: users } = await adminSupabase.from('users').select('id, role, position, chapter_position').limit(50);
  console.log(users.map(u => ({ role: u.role, pos: u.position, cpos: u.chapter_position })).filter(u => u.pos || u.cpos));
}
test();
