...
async function test() {
  const { data, error } = await adminSupabase.from('guest_invitations').select('attendance_status, attendance_updated_by, attendance_updated_by_name, attendance_updated_at').limit(1);
